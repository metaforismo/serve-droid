# Roadmap

## v0.1

- Local browser cockpit, typed CLI/HTTP/MCP surfaces, semantic UI tree, Logcat, app management,
  permissions, file transfer, cross-platform CI, and deterministic fixture.

## After v0.1 validation

- Native scrcpy transport performance hardening and an API 30+ opt-in audio matrix. Initial audio
  streaming is available now.
- Safari/Firefox decoder performance hardening. The TinyH264 compatibility path is available now.
- Rich trace export and browser recording controls. Bounded CLI opt-in H.264/event recording is
  available now.
- Named authenticated tunnel hardening and collaboration roles. Expiring foreground tunnel support
  is available now.
- AVD creation/provisioning and a multi-device grid. Installed AVD discovery and explicit
  start/stop lifecycle controls are available now.

Items move into a release only after a scoped issue defines security and acceptance criteria.
The detailed status and certification gates live in [docs/TODO.md](docs/TODO.md).
